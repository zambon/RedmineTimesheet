class TimesheetController < ApplicationController
  ...
  def save_timesheet
    ...
    # If timesheet_id is 0, we have a new timesheet
    if timesheet_id == 0
      t = Timesheet.new
  ...
  def edit_timesheet
    ...
    @projects = Project.find(:all, :order => 'name ASC', :conditions => Project.visible_by(User.current))    
    @enumerations = Enumeration.find(:all, :order => 'name ASC', :conditions => { :opt => "ACTI" })
    @issues = Issue.find(:all, :order => 'fixed_version_id, id', :conditions => ['project_id = ?', t.project_id])
    @date = params[:date]
  ...
  def get_issues
    ...
    @issues = Issue.find(:all, :order => 'fixed_version_id, id', :conditions => ['project_id = ?', project_id])
